...
int HandleValue(ASTPool *AST_Pool, ASTNodeStack *AST_Stack, TokenArray tokenArr, size_t *currentIndex, int *expressionDepth)
{
        ...
        if (!valueNode)
                return 1;
        if (PushASTNode(AST_Stack, valueNode))
                return 1;
        return ReduceTheStack(AST_Pool, AST_Stack, *expressionDepth);
}
...
int HandleBracket(ASTPool *AST_Pool, ASTNodeStack *AST_Stack, TokenArray tokenArr, size_t *currentIndex, int *expressionDepth)
{
        ...
        {
                PrintErrorf("Mismatched parentheses at line %zu, column %zu: expected depth higher than 0 but got %d", token.position.line, token.position.column, *expressionDepth); //@Error[Parser].
                return 1;
        }
        return 0;
}